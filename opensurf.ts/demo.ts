...
module OpenSURFDemo{
    ...
    $(()=>{
        ...
        function setImage(image:HTMLImageElement){
            ...
            $('#image_description').text("Image Size:" + img.width + "x" + img.height);
            $('#message').text("Ready.");
        }
        ...
        if(File){
            var selector:JQuery = $('#FileSelector');
            $('#FileSelector').change(()=>{
                var selectorElement:any = selector[0];
                if(selectorElement.files.length > 0){
                    var file:File = selectorElement.files[0];
            ...
            $('#OpenLocalFile').attr('disabled', 'disabled');            
        }

        setImageURL("yamada.png");
    });
} 
